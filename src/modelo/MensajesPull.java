...
import vista.VistaAllPulls;
import vista.VistaMyPulls;
...
public class MensajesPull {
	...
	public void login(String usuario) {
		try {
			sent = ejecutarConsulta(String.format(
					"SELECT userName FROM users WHERE userName LIKE '%s'",
					usuario));

			if (!sent.next()) {
				ejecutarSentence(String.format(
						"INSERT INTO users VALUES(null, '%s')", usuario));
			}
			showViews(usuario);
	...
	public void showViews(String usuario) {
		VistaAllPulls vap = VistaAllPulls.getVap();
		vap.showView(true);
		VistaPrincipal vp = VistaPrincipal.getVp();
		vp.showView(false);
		VistaMyPulls vmp = VistaMyPulls.getVmp();
		vmp.showView(true);

		sent = ejecutarConsulta(String.format(
				"SELECT * FROM users WHERE userName LIKE '%s'", usuario));
	...
	public void sentMessage(String message, int idUser) {
		ejecutarSentence(String
				.format("INSERT INTO messages VALUES(null, %d, curdate(), curtime(),'%s')",
						idUser, message));
	}
	...
	public DefaultListModel<String> llenarTablaAllPulls() {
		...
		String mensaje = "";
		try {
			sent = ejecutarConsulta("SELECT userName, date_format(dateMessage, '%y/%m/%d'), date_format(timeMessage, '%H:%i'), message FROM users "
					+ "INNER JOIN messages ON messages.idUser=users.idUser");
			while (sent.next()) {
				Object[] fila = new Object[1];
				for (int i = 0; i < fila.length; i++) {
					mensaje = "<html><body><p>" + sent.getString(1) + " "
							+ sent.getObject(2) + " " + sent.getObject(3)
							+ "</p><p>" + sent.getObject(4)
							+ "<p><br></body></html>";
				}
	...
	public DefaultListModel<String> llenarTablaMyPulls(int idUsuario) {
		...
		try {
			sent = ejecutarConsulta("SELECT userName, date_format(dateMessage, '%y/%m/%d'), date_format(timeMessage, '%H:%i'), message FROM users "
					+ "INNER JOIN messages ON messages.idUser=users.idUser WHERE users.idUser = "
					+ idUsuario);
			while (sent.next()) {
				Object[] fila = new Object[1];
				for (int i = 0; i < fila.length; i++) {
